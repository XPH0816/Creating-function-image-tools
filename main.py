...
class Picture:
    ...
    def __init__(self):
        ...
        # Get the Font
        self.fnt = ImageFont.truetype('font\FreeSerif.ttf', 44)
        self.fnt_i = ImageFont.truetype('font\FreeSerifItalic.ttf', 44)

        
    def DrawPoint(self,  function_type: int, function_name: str, num: int, point_domain: list, point_range: list, link_order: list):
        # Get the img size
        x, y = self.img.size
        ...
        left_text_x = (left_x1+11)
        left_text_y = (left_y1-10)
        
        # First Point Right Label position
        right_text_x = (right_x1-35)
        right_text_y = (right_y1-10)
        
        # Function name Position
        function_x = (x/2)
        function_y = (left_y1-20)

        # First Domain Value Position
        domain_x = left_x1 - 2
        domain_y = (left_y1+left_y2)/(num*2) if num > 2 else (left_y1+left_y2)/(num*2+0.25)
        
        # First Range Value Position
        range_x = right_x1 - 10
        range_y = (right_y1+right_y2)/(num*2) if num > 2 else (right_y1+right_y2)/(num*2+0.25)

        # Creating Draw Tools for the picture
        self.draw = ImageDraw.Draw(self.img)

        self.draw.ellipse((left_start_point, left_end_point), fill=(255, 255, 255), outline=(0, 0, 0), width=3)
        self.draw.ellipse((right_start_point, right_end_point), fill=(255, 255, 255), outline=(0, 0, 0), width=3)

        # Place the text
        if function_type == 0 :
            self.draw.text((left_text_x, left_text_y), "A", font=self.fnt, fill=(0, 0, 0, 255))
            self.draw.text((right_text_x, right_text_y), "B", font=self.fnt, fill=(0, 0, 0, 255))
            self.draw.text((function_x, function_y), "%s" % function_name, font=self.fnt_i, fill=(0, 0, 0, 255))
        if function_type == 1 :
            self.draw.text((left_text_x, left_text_y), "x", font=self.fnt_i, fill=(0, 0, 0, 255))
            self.draw.text((right_text_x, right_text_y), "%s(x)" % function_name, font=self.fnt_i, fill=(0, 0, 0, 255))
            self.draw.text((function_x, function_y), "%s" % function_name, font=self.fnt_i, fill=(0, 0, 0, 255))
        if function_type == 2 :
            self.draw.text((function_x, function_y), function_name, font=self.fnt_i, fill=(0, 0, 0, 255))
        
        #Draw the point
        for x in range(num):
            CONST = 245 - 45 * num
            self.draw.ellipse((left_first_start_x, left_first_start_y+(CONST*x), left_first_end_x, left_first_end_y+(CONST*x)), fill=(0, 0, 0))
            self.draw.ellipse((right_first_start_x, right_first_start_y+(CONST*x), right_first_end_x, right_first_end_y+(CONST*x)), fill=(0, 0, 0))
        
        # Set the FontSize
        self.fnt = ImageFont.truetype('font\FreeSerif.ttf', 28)

        # 2. 35 3.80 4. 120
        
        RANGE_CONST = 35 if num == 2 else 40*(num-1)
        ...
        for i, value in enumerate(point_domain, 1):
            ...
            if self.CheckInt(value):
                self.draw.text((domain_x-15*(j-1), domain_y*(i)+RANGE_CONST), value, font=self.fnt, fill=(0, 0, 0, 255))
            else :
                self.draw.text((domain_x-15*(j-1), domain_y*(i)+RANGE_CONST), value, font=self.fnt_i, fill=(0, 0, 0, 255))
        ...
        for i, value in enumerate(point_range, 1):
            ...
            if self.CheckInt(value):
                if i <= 2:
                    self.draw.text((range_x+20*(j-1), range_y*(i)+RANGE_CONST), value, font=self.fnt, fill=(0, 0, 0, 255))
                elif i > 2:
                    self.draw.text((range_x+20*(j-1), range_y*(i)+RANGE_CONST), value, font=self.fnt, fill=(0, 0, 0, 255))
            else :
                if i <= 2:
                    self.draw.text((range_x+20*(j-1), range_y*(i)+RANGE_CONST), value, font=self.fnt_i, fill=(0, 0, 0, 255))
                elif i > 2:
                    self.draw.text((range_x+20*(j-1), range_y*(i)+RANGE_CONST), value, font=self.fnt_i, fill=(0, 0, 0, 255))
    ...
    def DrawFunction(self, function_type: int, function_name: str, number_point: int, point_domain: list, point_range: list, link_order: list):
        ...
        for i, value in enumerate(point_domain, 1):
            ...
            if self.CheckInt(value):
                self.draw.text((domain_x-15*(j-1), domain_y*(i)), value, font=self.fnt, fill=(0, 0, 0, 255))
                self.draw.line((label_line_x1, domain_y*(i+CONST), label_line_x2, domain_y*(i+CONST)), fill=(0, 0, 0), width=2)
            else :
                self.draw.text((domain_x-15*(j-1), domain_y*(i)), value, font=self.fnt_i, fill=(0, 0, 0, 255))
                self.draw.line((label_line_x1, domain_y*(i+CONST), label_line_x2, domain_y*(i+CONST)), fill=(0, 0, 0), width=2)
        ...
        for i, value in enumerate(point_range, 1):
            ...
            if self.CheckInt(value):
                if i <= 2:
                    self.draw.text((range_x+20*(j-1), range_y*(i+CONST/i)), value, font=self.fnt, fill=(0, 0, 0, 255))
                    self.draw.line((label_line_x3, range_y*(i+0.25+CONST/i), label_line_x4, range_y*(i+0.25+CONST/i)), fill=(0, 0, 0), width=2)
                elif i > 2:
                    self.draw.text((range_x+20*(j-1), range_y*(i-CONST/(5-i))), value, font=self.fnt, fill=(0, 0, 0, 255))
                    self.draw.line((label_line_x3, range_y*(i+0.25-CONST/(5-i)), label_line_x4, range_y*(i+0.25-CONST/(5-i))), fill=(0, 0, 0), width=2)
            else :
                if i <= 2:
                    self.draw.text((range_x+20*(j-1), range_y*(i+CONST/i)), value, font=self.fnt_i, fill=(0, 0, 0, 255))
                    self.draw.line((label_line_x3, range_y*(i+0.25+CONST/i), label_line_x4, range_y*(i+0.25+CONST/i)), fill=(0, 0, 0), width=2)
                elif i > 2:
                    self.draw.text((range_x+20*(j-1), range_y*(i-CONST/(5-i))), value, font=self.fnt_i, fill=(0, 0, 0, 255))
                    self.draw.line((label_line_x3, range_y*(i+0.25-CONST/(5-i)), label_line_x4, range_y*(i+0.25-CONST/(5-i))), fill=(0, 0, 0), width=2)
...
if __name__ == '__main__':
    ...
    img.DrawPoint(0, "f", 3, ["1","2","3"], ["1","2","3"], [[1,2],[2,1],[3,2]])
    img = img.img
    img.show()
